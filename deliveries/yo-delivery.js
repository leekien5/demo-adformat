...
var avlProtocal = (document.location.protocol == 'https:') ? 'https://' : 'https://';
...
var avlInteractModule = new function(){
	...
	return {
		...
		initVideoInlineVPaid: function(pid, elw, elh, el, w, z, fp, a, az, autoStart, v, d, cs, ovr){
            ...
            domWrapPlayer.appendChild(domPlayerAds);
            domWrapPlayer.appendChild(domPlayerInner);
            var domSkipAd = domManipulate.create('div', 'skipad'+el, 'display:none;z-index:999999;', '<a style="z-index:999999;display:block;position: absolute;top: 0px;left: '+(parseInt(elw) - 79)+'px;" onclick="skipAds();" href="javascript:;"><img src="https://static.yomedia.vn/2015/09/skip-ad.png" /></a>');
		...
		initBanner: function(a, b, w, h, f, fv, s, m) {
	        ...
	        close.setAttribute('src', 'https://static.yomedia.vn/2015/08/btn_close.png');
		...
		init : function(initVar){
			if (typeof initVar !== 'undefined' && navigator.cookieEnabled) {
			    if (initVar.length > 0) {
				    ...
				    if (initVar[lastIndexInitVar][0].length != 32) {
				    	var pid = initVar[lastIndexInitVar][1];
				    	var div = initVar[lastIndexInitVar][3];
				    	var w = initVar[lastIndexInitVar][4];
				    	var h = initVar[lastIndexInitVar][5];
				    } else {
				    	var pid = initVar[lastIndexInitVar][0];
				    	var div = initVar[lastIndexInitVar][1];
				    	var w = initVar[lastIndexInitVar][2];
				    	var h = initVar[lastIndexInitVar][3];
				    }
					var varObj 				= {
						"pid": pid,
						"div": div,
